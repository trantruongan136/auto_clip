...
const TaskProgressModal: React.FC<TaskProgressModalProps> = ({
  ...
  const steps = [
    { title: 'Outline extraction', description: 'Extract structural outlines from video transcribed text' },
    { title: 'time positioning', description: 'based onSRTSubtitle positioning topic time interval' },
    { title: 'Content rating', description: 'Multi-dimensional evaluation of clip quality and dissemination potential' },
    { title: 'Title generation', description: 'Generate catchy titles for high-scoring snippets' },
    { title: 'topic clustering', description: 'Aggregate related fragments into collection recommendations' },
    { title: 'Video cutting', description: 'useFFmpegGenerate sliced ​​and aggregated videos' }
  ]
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        ...
        // If processing is completed, notify the parent component
    ...
    // If the task is still in progress, check the status periodically
  ...
  const handleRetry = async () => {
    ...
    try {
      if (status?.current_step !== undefined) {
        // Try again from the current step
          ...
          <Space direction="vertical" size="large" style={{ width: '100%' }}>
            ...
            {status.status === 'completed' && (
              <Alert
                ...
                description="The video was successfully processed and you can view the resulting clips and collections."
                type="success"
                showIcon
              />
            )}
